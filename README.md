...
| Transects | ✅ deterministic mixed fields | ✅ ensemble-native mixed fields | ✅ deterministic mixed fields | ✅ member-first mixed fields |
...
GEFS now also has an explicit **GEFSv12 reforecast** branch. It remains public `dataset: "gefs"`, but callers set `forecast.kind: "reforecast"` and an explicit historical 00Z run. Capability discovery mirrors that distinction: `wfg catalog --dataset gefs --forecast-kind reforecast` (or MCP `search_catalog` with `datasets: ["gefs"]`, `forecastKind: "reforecast"`) returns only the retrospective subset rather than operational-only capabilities. Reforecasts are retrospective forecasts produced with a consistent GEFSv12 system; they are **not archived operational GEFS runs**. Point and multi-point access covers supported single-level fields plus native pressure profiles for temperature, U/V wind, geopotential height, vertical velocity and specific humidity from the NOAA AWS retrospective corpus (2000–2019), using the standard five-member daily set `c00,p01..p04` by default. Fields and pressure variables can now be mixed in the same request, including bounded point and multi-point ranges with compact ensemble summaries. WFG preserves the archive's native cadence and pressure storage: through f240 the retrospective output is 3-hourly, with levels at/below 700 hPa in 0.25° base files and upper levels in 0.5° files; from f246 through f384 the standard horizon is 0.5°/6-hour output. Range results therefore keep grid point and grid provenance per step rather than pretending the resolution is invariant across the day-10 boundary. Mixed selections go one step further: because retrospective field files and pressure files can have different native horizontal grids, WFG reports separate pressure and field grid points/provenance instead of fabricating one common mesh. Raw member payloads remain single-time only. The same retrospective branch now supports all three canonical layer diagnostics (`temperature_lapse_rate`, `wind_shear`, `potential_temperature_gradient`) and both structural profile diagnostics (`freezing_level_crossings`, `temperature_inversion_layers`) at one valid time or across bounded native-cadence ranges. These are derived member-by-member before ensemble aggregation and preserve retrospective grid provenance per step. Parcel diagnostics remain explicitly unsupported because the current reforecast subset does not expose the moisture/surface inputs required by the parcel kernel. Transect/area geometry, derived profile thermodynamics, calibration metrics and the weekly +35-day extension remain explicit follow-ups rather than being guessed.
