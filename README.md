...
GEFS now also has an explicit **GEFSv12 reforecast** branch. It remains public `dataset: "gefs"`, but callers set `forecast.kind: "reforecast"` and an explicit historical 00Z run. Capability discovery mirrors that distinction: `wfg catalog --dataset gefs --forecast-kind reforecast` (or MCP `search_catalog` with `datasets: ["gefs"]`, `forecastKind: "reforecast"`) returns only the retrospective subset rather than operational-only capabilities. Reforecasts are retrospective forecasts produced with a consistent GEFSv12 system; they are **not archived operational GEFS runs**. Point/instant access covers supported single-level fields plus native pressure profiles for temperature, U/V wind, geopotential height, vertical velocity and specific humidity from the NOAA AWS retrospective corpus (2000–2019), using the standard five-member daily set `c00,p01..p04` by default. WFG preserves the archive's native pressure storage: through day 10, levels at/below 700 hPa use 0.25° files while upper levels use 0.5° files; mixed profiles are sampled on a common 0.5° column. After day 10 the standard horizon uses 0.5°/6-hour output. Ranges, spatial geometries, diagnostics, derived profile thermodynamics, calibration metrics and the weekly +35-day extension remain explicit follow-ups rather than being guessed.
