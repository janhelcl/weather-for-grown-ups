...
export interface AigefsForecastServiceOptions {
  cacheDir?: string;
  concurrency?: number;
  memberServiceFactory?: (member: AigefsMember) => AigefsMemberService;
}
...
export class AigefsForecastService {
  ...
  private readonly memberServiceFactory: (member: AigefsMember) => AigefsMemberService;

  constructor(options: AigefsForecastServiceOptions = {}) {
    const cacheDir = options.cacheDir
      ?? process.env.WFG_CACHE_DIR
      ?? join(homedir(), ".cache", "wfg");
    this.concurrency = options.concurrency ?? DEFAULT_AIGEFS_MEMBER_CONCURRENCY;
  ...
  async diagnose(request: DiagnoseAtmosphereRequest): Promise<unknown> {
    ...
    if (request.diagnostic.kind === "parcel") {
      throw new Error(
        "AIGEFS does not expose parcel diagnostics because its member surface product lacks the required parcel initialization state",
      );
    }
    const members = requestedMembers(request.ensemble?.members);
    const quantiles = requestedQuantiles(request.ensemble?.quantiles);
...
function requestedMembers(input: readonly string[] | undefined): AigefsMember[] {
  const raw = input ?? AIGEFS_MEMBERS;
  const unsupported = raw.filter((member) => !MEMBER_SET.has(member));
  if (unsupported.length > 0) {
    throw new Error(
      `AIGEFS members are c00,p01..p30; unsupported: ${unsupported.join(", ")}`,
    );
  }
  if (raw.length < 2) throw new Error("AIGEFS requires at least two selected members");
  return sortAigefsMembers(raw as AigefsMember[]);
}
...
function summarizePointsInstant(
  ...
  return {
    ...
    points: first.points.map((point: any, index: number) => {
      ...
      assertGridPointsEqual(memberPoints.map((candidate: any) => candidate.gridPoint), "AIGEFS points");
      return {
...
function summarizeTransect(
  ...
  for (const entry of members) {
    if (entry.result.samples.length !== sampleCount) {
      throw new Error("AIGEFS member transects returned inconsistent sample counts");
    }
  }
  return {
    model: MODEL,
      ...
      distribution: summarizeNumericDistribution(
        members.map(({ result: memberResult }) =>
          requiredNumber(
            ...
            `AIGEFS member spatial percentile ${percentile}`,
          )),
        quantiles,
      ),
    }));
      ...
      distribution: summarizeNumericDistribution(
        members.map(({ result: memberResult }) =>
          requiredNumber(
            ...
            `AIGEFS member spatial threshold ${threshold.operator} ${threshold.value}`,
          )),
        quantiles,
      ),
      interpretation:
  ...
  if (request.aggregate?.includeExtremaLocations === true) {
    result.memberExtrema = members.map(({ member, result: memberResult }) => ({
      ...
      ...requiredObject(
        ...
        `AIGEFS member extrema for ${member}`,
      ),
    }));
  }
...
function profileSummaries(results: readonly any[], quantiles: readonly number[]) {
  ...
  for (const result of results) {
    if ((result.levels?.length ?? 0) !== levels.length) {
      throw new Error("AIGEFS member profiles returned inconsistent pressure-level counts");
    }
  }
    ...
    return fields.map((field) => {
      const values = results.map((result) =>
        requiredNumber(
          ...
          `AIGEFS profile ${field}@${level.pressureHpa}mb`,
        ));
...
function aggregateFields(
  ...
  for (const fields of members) {
    if (fields.length !== first.length) {
      throw new Error("AIGEFS member field bundles returned inconsistent field counts");
    }
  }
  return first.map((field, fieldIndex) => {
    ...
    for (const candidate of candidates) {
      if (
        ...
        throw new Error(`AIGEFS member field metadata disagree for ${field.id}`);
      }
    }
    const outputs = Object.keys(field.values).map((output) => {
      const values = candidates.map((candidate) =>
        requiredNumber(candidate.values[output], `AIGEFS ${field.id}.${output}`));
...
function ensembleSource(members: readonly MemberResult[]) {
  return {
    provider: "NOAA EAGLE AWS Open Data" as const,
...
function assertSameSeriesLength(members: readonly MemberResult[]): void {
  ...
  if (!Number.isInteger(length)) throw new Error("AIGEFS member result is missing series");
  for (const { result } of members) {
    if (result.series?.length !== length) {
      throw new Error("AIGEFS members returned inconsistent time-series lengths");
    }
  }
}
...
function assertSamePointCount(
  ...
  for (const { result } of members) {
    if (select(result).length !== count) {
      throw new Error("AIGEFS members returned inconsistent point counts");
    }
  }
}
...
function assertCommonPointGrid(
  ...
  assertGridPointsEqual(members.map(select), "AIGEFS members");
}
...
function requiredNumber(value: unknown, context: string): number {
  if (typeof value !== "number" || !Number.isFinite(value)) {
    throw new Error(`Internal AIGEFS aggregation is missing ${context}`);
  }
  return value;
}
