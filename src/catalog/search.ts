...
interface SearchableCatalogEntry extends Omit<CatalogSearchMatch, "score"> {
  searchParts: string[];
}

export function searchGfsCatalog(input: CatalogSearchQueryInput = {}): CatalogSearchResult {
  ...
  const entries = buildSearchEntries().filter((entry) => {
    if (!allowedSections.has(entry.section)) return false;
  ...
  const scored = entries
    ...
    .filter((value): value is { entry: SearchableCatalogEntry; score: number } => value !== null)
    .sort((left, right) =>
...
function buildSearchEntries(): SearchableCatalogEntry[] {
  const variables: SearchableCatalogEntry[] = Object.values(VARIABLE_CATALOG).map((definition) => ({
    section: "variables",
    id: definition.id,
    ...
    searchParts: definition.kind === "raw"
      ? [definition.gfsCode, definition.sourceUnit]
  ...
  const fields: SearchableCatalogEntry[] = GFS_NON_ISOBARIC_FIELD_IDS
    ...
    outputs: definition.outputs.map((output) => ({ ...output })),
    searchParts: definition.kind === "raw"
      ? [definition.gfsCode, definition.sourceUnit, definition.level.nomadsLevel]
      : [...definition.dependencies, definition.level.nomadsLevel],
  }));

  const layerDiagnostics: SearchableCatalogEntry[] = Object.values(LAYER_DIAGNOSTIC_CATALOG).map((definition) => ({
  ...
  const profileDiagnostics: SearchableCatalogEntry[] = Object.values(PROFILE_DIAGNOSTIC_CATALOG).map((definition) => ({
  ...
  const parcelDefinitions: SearchableCatalogEntry[] = Object.values(PARCEL_DIAGNOSTIC_CATALOG).map((definition) => ({
    ...
    dependencies: [...new Set([...definition.pressureDependencies, ...definition.fieldDependencies])],
    outputs: definition.outputs.map((output) => ({ ...output })),
    searchParts: [...definition.pressureDependencies, ...definition.fieldDependencies],
  }));
...
function searchScore(entry: SearchableCatalogEntry, rawSearch: string | undefined): number | null {
