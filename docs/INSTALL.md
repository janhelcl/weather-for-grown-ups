...
The npm package includes its GRIB2 decoder for every dataset, so native weather tooling is not required on the host. This includes ICON-D2-EPS: DWD exposes that ensemble on its provider-native triangular grid, and WFG downloads DWD's official target-grid/weights bundle once and applies the same nearest-neighbour remap in-process before splitting members by their GRIB2 perturbation number. Values are identical to DWD's documented CDO conversion path because the same provider index table is used.
...
The package's default GRIB2 engine is bundled through npm and serves every dataset, including `icon-d2-eps`. Native `wgrib2` is an optional compatibility/debug backend selected only through `WGRIB2_PATH` or `WFG_DECODER=wgrib2`; nothing requires it.
...
Docker remains useful for pinned/reproducible deployments. The image contains Node.js 24 and native `wgrib2 3.8.0` from conda-forge as the optional compatibility/debug decoder backend.
