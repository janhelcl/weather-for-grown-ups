...
The default member selection is all 50 perturbations. Point and multi-point time ranges pin one initialization across the complete range, preserve the native ENS cadence (3-hourly through f144, then 6-hourly on 00/12Z long runs), and return compact distributions at each point/step by default. Multi-point composition preserves input ordering and applies explicit point × step and point × step × member × scalar-output guardrails before dispatch; raw member payloads remain opt-in and size-guarded. Instant layer, whole-profile and parcel diagnostics are also member-first: physics is derived independently for each perturbation, then summarized with distributions or raw-member event fractions. Diagnostic time series preserve the same native ENS cadence and pin one initialization across the requested range; they return compact member-first summaries at each step. Great-circle transects are composed from the same bounded multi-point primitive and therefore preserve the same member-first distributions, optional raw perturbation payloads and point-count guardrails. Scalar area statistics are also ensemble-native: WFG computes mean/min/max, requested spatial percentiles, threshold fractions and extrema independently inside every perturbation, then summarizes those member-level spatial statistics across the ensemble. Bbox and member × grid-point work are guarded before source access; raw grid values are never returned. Run comparison is ensemble-native: each cycle is summarized independently and only distribution shifts are compared, with 6-hour or 12-hour initialization stride.
