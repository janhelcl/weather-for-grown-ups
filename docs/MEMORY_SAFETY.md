...
2. **Bundled point decoding is message-streamed with bounded CPU overlap.** When a point request does use a source that retains full grids, bundled decoding parses, samples and releases GRIB messages instead of materializing every requested variable/level grid together. Independent messages may unpack on a process-local worker pool (CPU overlap only; no extra provider access), but the default pool is capped at two workers so CPU parallelism does not scale memory use with host core count. Multi-point sampling unpacks each message once. `WFG_GRIB_DECODE_WORKERS` remains an explicit operator override.
