...
| `pe-arome` | `pe_arome_0p025` Météo-France PE-AROME targeted WCS | forecast | 25-member regional field ensemble |
...
`pe-arome` preserves the Météo-France control + 24-perturbation population (`c00,p01..p24`) and hourly f51 regional semantics. The current explicitly mapped WCS slice exposes 2 m temperature and relative humidity across point/range/multi-point/transect geometry plus native scalar area summaries. WFG resolves one run, normalizes each selected member first, then aggregates field distributions. Wind and pressure capabilities remain unadvertised until confirmed against the subscribed PE service. Provider bearer auth and one-member/one-field WCS packaging stay in the source layer. See [PE_AROME.md](PE_AROME.md).
...
| PE-AROME operational | `latest`, `latest_complete`, explicit ISO cycle |
...
For `icon-d2`, DWD's deterministic pressure-level objects expose 200/250/300/400/500/600/700/850/950/975/1000 hPa. `icon-d2-eps` has a narrower native pressure inventory: 500/700/850/950/975/1000 hPa. WFG keeps those catalogs separate, uses 1000/950/850/700/500 hPa for the default lower-atmosphere profile shared by both datasets, and probes every concrete selected pressure product and field when resolving a live run. Unsupported levels therefore fail during capability validation rather than later as provider 404s.
