...
| `compare_datasets` | Compare aligned datasets: GFS↔GEFS context, deterministic GFS↔IFS deltas, GEFS↔IFS ENS distribution shifts, or deterministic IFS control vs its IFS ENS perturbation distribution |
...
`search_catalog` defaults to operational capabilities. To plan a GEFSv12 retrospective query, pass `datasets: ["gefs"]` and `forecastKind: "reforecast"`. The result then contains only capabilities currently exposed by the reforecast query path: the verified retrospective single-level fields and six native pressure variables. Operational-only derived thermodynamics, layer/profile diagnostics and parcel definitions are omitted rather than advertised and rejected later. The CLI equivalent is `wfg catalog --dataset gefs --forecast-kind reforecast`.
