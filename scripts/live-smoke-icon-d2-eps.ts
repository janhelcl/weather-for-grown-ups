...
try {
  ...
  await dumpInventories(diagnosticCacheDir);
  throw error;
}
...
async function dumpInventories(cacheDir: string): Promise<void> {
  for (const subdir of [
    "icon-d2-eps-open-data",
    "icon-d2-eps-remapped",
    "icon-d2-eps-members",
    "icon-d2-eps-member-combined",
  ]) {
    ...
    for (const file of files.filter((name) => name.endsWith(".grib2"))) {
      ...
      const relevant = lines.filter((line) =>
        /RAIN|SNOW|parmcat=1|parm=55|parm=76|BREF|REFC|DBZ|TMP:850/i.test(line));
      console.error(
        `WFG EPS STAGE ${subdir}/${file} records=${lines.filter(Boolean).length}\n`
