...
const indexText = [
  ...
  "6:40:d=2026081906:HGT:surface:6 hour fcst:",
].join("\n");

const chunks: Record<string, string> = {
  "bytes=0-7": "GRIB0000",
  "bytes=8-15": "GRIB1111",
  "bytes=16-23": "GRIB2222",
  "bytes=24-31": "GRIB3333",
  "bytes=32-39": "GRIB4444",
};

let rootDir: string;

beforeEach(async () => {
  rootDir = await mkdtemp(join(tmpdir(), "wfg-s3-"));
});
...
function request(variables = ["temperature", "relative_humidity"] as const, levels = [850]): ProfileDataRequest {
  return {
    ...
    latitude: 50.08,
    longitude: 14.43,
    variables: expandRequestedVariables([...variables]),
...
function makeFetch() {
  return vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (url.endsWith(".idx")) return new Response(indexText, { status: 200 });
...
describe("GfsS3SubsetCache", () => {
  it("fetches the index, downloads only selected GRIB byte ranges, and concatenates messages", async () => {
    ...
    const indexCalls = fetchFn.mock.calls.filter(([input]) => String(input).endsWith(".idx"));
    expect(indexCalls).toHaveLength(1);
    const rangeHeaders = fetchFn.mock.calls
      .map(([, init]) => new Headers(init?.headers).get("range"))
      .filter(Boolean);
    expect(rangeHeaders).toEqual(["bytes=0-7", "bytes=8-15"]);
  });
  ...
  it("fails on an unavailable index", async () => {
    ...
    const cache = new GfsS3SubsetCache(rootDir, fetchFn as typeof fetch);
    await expect(cache.fetch(request())).rejects.toThrow(/index request failed: HTTP 503/);
  });
  ...
  it("fails rather than accepting a server that ignores the Range header", async () => {
    const fetchFn = vi.fn(async (input: string | URL | Request) => {
      ...
      return new Response(new TextEncoder().encode("GRIB0000"), { status: 200 });
    });
    const cache = new GfsS3SubsetCache(rootDir, fetchFn as typeof fetch);
    await expect(cache.fetch(request(["temperature"], [850]))).rejects.toThrow(/range request failed: HTTP 200/);
  });

  it("rejects byte ranges that do not begin with a GRIB message", async () => {
    const fetchFn = vi.fn(async (input: string | URL | Request) => {
      if (String(input).endsWith(".idx")) return new Response(indexText, { status: 200 });
      return new Response(new TextEncoder().encode("NOPE0000"), { status: 206 });
    });
    const cache = new GfsS3SubsetCache(rootDir, fetchFn as typeof fetch);
    await expect(cache.fetch(request(["temperature"], [850]))).rejects.toThrow(/did not start with a GRIB message/);
  });

  it("fails before range downloads when the index has no matching field", async () => {
    const fetchFn = makeFetch();
    const cache = new GfsS3SubsetCache(rootDir, fetchFn as typeof fetch);
    await expect(cache.fetch(request(["temperature"], [925]))).rejects.toThrow(/No matching pressure-level fields/);
