...
describe("parseWgrib2PointLine", () => {
  it.each(["TMP", "RH", "UGRD", "VGRD"] as const)("parses supported code %s", (code) => {
  ...
  it("parses scientific notation and fractional pressure levels", () => {
    const line = "1:1:d=2026081906:UGRD:850.5 mb:6 hour fcst:lon=359.75,lat=-12.5,val=-1.2e+03";
    expect(parseWgrib2PointLine(line)).toEqual({
      code: "UGRD",
      pressureHpa: 850.5,
      value: -1200,
      gridPoint: { longitude: -0.25, latitude: -12.5 },
    });
  });

  it("keeps 180 degrees signed longitude unchanged", () => {
    const line = "1:1:d=2026081906:TMP:500 mb:6 hour fcst:lon=180,lat=0,val=250";
    expect(parseWgrib2PointLine(line)?.gridPoint.longitude).toBe(180);
  });

  it.each([
    "",
    "1:0:d=2026081906:HGT:850 mb:6 hour fcst:lon=14.5,lat=50,val=1500",
...
describe("Wgrib2Decoder", () => {
  it("invokes wgrib2 with -s -lon and converts negative longitude to 0-360", async () => {
    execaMock.mockResolvedValue({
      stdout: "1:1:d=2026081906:TMP:850 mb:6 hour fcst:lon=350,lat=50,val=285.4",
    } as never);

    const decoder = new Wgrib2Decoder("/opt/wgrib2");
    const values = await decoder.extractPoint("/tmp/test.grib2", -10, 50);
  ...
  it("parses multiple supported output lines and skips noise", async () => {
    execaMock.mockResolvedValue({
      stdout: [
        "noise",
        "1:1:d=2026081906:TMP:850 mb:6 hour fcst:lon=14.5,lat=50,val=285.4",
        "2:2:d=2026081906:RH:850 mb:6 hour fcst:lon=14.5,lat=50,val=65",
      ].join("\n"),
    } as never);
  ...
  it("fails clearly when wgrib2 returns no supported point values", async () => {
    ...
    await expect(new Wgrib2Decoder().extractPoint("field.grib2", 14.5, 50)).rejects.toThrow(
      /no supported point values/,
    );
  });
  ...
  it("translates ENOENT into an actionable installation error", async () => {
    ...
    await expect(new Wgrib2Decoder().extractPoint("field.grib2", 14.5, 50)).rejects.toThrow(
      /wgrib2 is required but was not found/,
    );
  });
